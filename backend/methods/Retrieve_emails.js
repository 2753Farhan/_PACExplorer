...
export async function retrieveEmailsByJobId(jobId) {
    const client = new MongoClient(uri);
    ...
    try {
        ...
        // Retrieve documents based on job ID and project only the email field
        const cursor = collection.find({ jobId }, { projection: { email: 1 } });
